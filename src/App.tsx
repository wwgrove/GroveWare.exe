  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-black text-white">
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <video autoPlay loop muted playsInline className="absolute min-w-full min-h-full object-cover top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-45 mix-blend-screen">
          <source src={backgroundVideo} type="video/mp4" />
        </video>
        <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-black/35 to-black/85 pointer-events-none" />
      </div>
      <div className="flex-1 flex flex-col items-center justify-center px-4">
        <div className="max-w-4xl w-full text-center space-y-16">
          <div className="space-y-6">
            ...
            <p className="text-slate-300 text-xs md:text-sm tracking-[0.1em] max-w-lg mx-auto uppercase font-mono leading-relaxed">Some questions don't have answers. Some advantages don't have explanations. You either know, or you don't.</p>
          </div>
          {submitted ? (
            <div className="max-w-md mx-auto space-y-6 relative z-10 w-full mt-12 bg-black/25 p-8 backdrop-blur-md border border-slate-700/60">
              ...
              <div className="mt-8 pt-6 border-t border-slate-700"><p className="text-slate-400 text-[9px] tracking-[0.1em] font-mono uppercase">Await further instructions.</p></div>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="max-w-md mx-auto space-y-6 relative z-10 w-full mt-12 bg-black/25 p-8 backdrop-blur-md border border-slate-700/60">
              <h2 className="text-slate-200 text-[10px] tracking-[0.3em] uppercase font-mono text-center mb-8">Request Secure Link</h2>
              ...
              <button type="submit" disabled={isLoading} className="w-full mt-8 bg-transparent border border-slate-600 text-slate-100 font-mono text-xs tracking-widest uppercase py-4 hover:bg-[#ff33cc] hover:text-black hover:border-[#ff33cc] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">{isLoading ? 'Transmitting...' : 'Transmit Request'}</button>
            </form>
          )}
        </div>
      </div>
      <footer className="absolute bottom-6 w-full text-center">
        <p className="text-slate-400 text-[10px] tracking-[0.2em] uppercase font-mono">© 2026 GroveWare. All rights reserved.</p>
      </footer>
    </div>
  );
}
